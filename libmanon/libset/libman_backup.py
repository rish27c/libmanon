            ...
            try:
                if '(' in scancmd[0] or ')' in scancmd[0] or ',' in scancmd[0] or '=' in scancmd[0] or '#' in scan:
                    ...
                    if ask==password:
                        cr=True
                        try:
                            liber_backup.core(scancmd, data)
                    ...
                    eval('liber_backup.'+scancmd[0])(scancmd, data)  #eval converts the input to function and call it in liber, makes it easy to create and handle commands
                elif cr==True:
                    eval('liber_backup.'+scancmd[0])(scancmd, data) #I found it annoying that it asked for password everytime, so I just stopped the redundancy
                else:
                    raise Exception #To trigger the print statement given below
            except AttributeError:
                print('\nUnknown command or mistype or not known by liber ¯\\_(ツ)_/¯\n', flush=True)
                liber_backup.sug(scancmd[0], ['slog', 'libman', 'core', 'libql', 'liberb'])
