...
def core(arg, data):
    from . import getpass
    try:
        pkey=getpass.getpass('(Not visible) Enter Verification Password: ')
    except:
        pkey=input('(Vulnerable) Enter Password: ')
    if data[2]=='#None*~':
        password=''
    else:
        password=data[2]
    if pkey!=password:
        print('Invalid Password: Verifiction failed!')
        raise ValueError('Core exited with error!')
    import os
    if len(arg)==1:
