...
def liber(arg, data):
    if len(arg)==1:
        ...
        print('Liber sees you are confused, try: "liber libhelp"! (ง•̀ᗝ•́)ง')
    else:
        if arg[1]=='libhelp': #Calls for help, duh!
            ...
            if 'liber' in cr:
                ...
                print('Authentication_Validation1: Not Validated\n\tcore hasn\'t permitted priviledge', flush=True)
        else:
            print('\n\tUnknown ¯\\_(ツ)_/¯\n', flush=True)
...
def core(arg, data, uf_call=False):
    if uf_call==True:
        print('Core Access point it not command! >_<')
        from .uf_var import le
        if '#spx_key_ok' in le:
            print('Already verified from uf: Skipping authentication...')
        else:
            from . import getpass
            try:
                pkey=getpass.getpass('[Not visible] Password: ')
            except:
                pkey=input('(Vulnerable) Password: ')
            if data[2]=='#None*~':
                password=''
            else:
                password=data[2]
    ...
    if len(arg)==1:
        ...
        if arg[1]=='show':
            ...
            if len(arg)>2:
                ...
                print('Core priviledge granted to liber')
        elif arg[1]=='libql':
            cr.append('libql')
            if len(arg)>2:
                libql(arg[1:], data)
            else:
                print('Core priviledge granted to libql')
        elif arg[1]=='*':
            cr.extend(['liber', 'libql'])
            if len(arg)>2:
                print('\nFlow->Undefined :(\n', flush=True)
            else:
                print('Core priviledge granted to libql, liber')
        else:
...
def libql(arg, data):
    if 'libql' in cr:
        ...
        print('Authentication_Validation1: Not Validated\nCore has not permitted priviledge escalation of libql :(', flush=True)

def call(arg, data):
    if len(arg)==1:
        print('\nLibCall: Extra Aguement! ૮₍ ˶>⤙<˶  ₎ა\n', flush=True)
    else:
        if arg[1]=='user-friendly' or arg[1]=='uf':
            if len(arg)>2:
                print('Too much argument! ! ૮₍ ˶>⤙<˶  ₎ა\n', flush=True)
            else:
                try:
                    from . import user_interface
                    try:
                        f2=user_interface.precompile()
                    except:
                        pass
                    try:
                        eval(str(f'{f2[0]}({f2}, {data})'))
                    ...
                    while True:
                        from . import user_interface
                        try:
                            f=user_interface.compile()
                            user_interface.lmvr={}
                        except Exception as err:
                            log(err, 'InterfaceHandler-Error')
                        try:
                            if f[0]=='core':
                                eval(str(f'{f[0]}({f}, {data}, True)'))
                            else:
                                eval(str(f'{f[0]}({f}, {data})'))
                            input('\nPress Enter to Continue...')
                        except:
                            pass
                        try:
                    ...
                    print('Failed to load user interaction module')
        else:
             print('LibCall: Unknown command', flush=True)